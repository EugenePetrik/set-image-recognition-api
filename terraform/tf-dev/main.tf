...
output "github_secrets" {
  description = "Values to add as GitHub Secrets (for DEV environment)"
  value = {
    # AWS Configuration
    AWS_REGION     = var.aws_region
    ...
    # ECS Configuration
    ECS_CLUSTER_DEV = module.application.ecs_cluster_name
    ECS_SERVICE_DEV = module.application.ecs_service_name

    # Lambda Configuration
    LAMBDA_FUNCTION_DEV = module.application.lambda_function_name

    # S3 Bucket
    S3_BUCKET_DEV = module.environment.s3_bucket_name

    # DynamoDB Table
    DYNAMODB_TABLE_DEV = module.environment.dynamodb_table_name

    # SNS Topic
    SNS_TOPIC_ARN_DEV = module.environment.sns_topic_arn

    # SQS Queue
    SQS_QUEUE_URL_DEV = module.environment.sqs_queue_url
  }
}
...
output "deployment_info" {
  description = "Key information for deployment and testing"
  value = {
    environment      = var.environment
    aws_region       = var.aws_region
    aws_account_id   = data.aws_caller_identity.current.account_id
