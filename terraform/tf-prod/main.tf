...
output "github_secrets" {
  description = "Values to add as GitHub Secrets (for PROD environment)"
  value = {
    # AWS Configuration
    AWS_REGION     = var.aws_region
    ...
    # ECS Configuration
    ECS_CLUSTER_PROD = module.application.ecs_cluster_name
    ECS_SERVICE_PROD = module.application.ecs_service_name

    # Lambda Configuration
    LAMBDA_FUNCTION_PROD = module.application.lambda_function_name

    # S3 Bucket
    S3_BUCKET_PROD = module.environment.s3_bucket_name

    # DynamoDB Table
    DYNAMODB_TABLE_PROD = module.environment.dynamodb_table_name

    # SNS Topic
    SNS_TOPIC_ARN_PROD = module.environment.sns_topic_arn

    # SQS Queue
    SQS_QUEUE_URL_PROD = module.environment.sqs_queue_url
  }
}
